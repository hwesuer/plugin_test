...
- **配置优先**：优先读取 AstrBot 已存储的配置值（通过管理面板修改的值）
- **Schema 回退**：若配置值为空，自动回退读取 `_conf_schema.json` 中的默认值
- **命令落盘**：执行 `/blockword add` / `blockword remove` 后持久化到 `data/astrbot_plugin_blockwords_data.json`，并标记为手动管理
- **手动管理**：一旦通过命令管理过关键词，数据文件将作为唯一数据源；配置变更需执行 `/blockword sync` 同步
